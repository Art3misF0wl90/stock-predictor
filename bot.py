...
def tool_explain_signal(ticker: str):
    ...
    earn     = None if ticker == "TSLA" else build_earnings_features(ticker, df)
    df_feat  = add_features(df, macro_df=macro_df,
                            sentiment_series=sent,
                            earnings_df=earn,
                            forward_days=cfg["fwd_days"],
                            predict_mode=True)
    if df_feat.empty:
    ...
    import joblib as jl
    scaler  = jl.load(os.path.join("models", f"{ticker}_scaler.pkl"))
    model   = jl.load(os.path.join("models", f"{ticker}_model.pkl"))
    feat_cols = [c for c in cfg["feat_cols"] if c in df_feat.columns]
    X       = df_feat[feat_cols].iloc[[-1]].values
    prob_up = float(model.predict_proba(scaler.transform(X))[0][1])
    return {
        "ticker":       ticker,
        "model":        cfg["model_name"],
        "horizon":      f"{cfg['fwd_days']}d",
